import { Tech Holding JSCStakingv2__factory } from "../../typechain";
...
export const currentIndex = async ({ chainId }: { chainId: ChainId }): Promise<number> => {
  ...
  const stakingContract = Tech Holding JSCStakingv2__factory.connect(Addresses[chainId].STAKING_V2, provider);
