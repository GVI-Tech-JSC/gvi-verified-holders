...
import { MINIMUM_GVI_EQUIV_AUTH, SEND_DM_REVOKE } from "../../config";
...
const balances = async (request: VercelRequest, response: VercelResponse) => {
  ...
  while (true) {
    ...
    // Array of users
    const usersBatch = _usersBatch.data.discord_gviies;
    // No more users
    ...
    await Promise.all(
      usersBatch.map(async (user: { address: string; chainId: ChainId; discordUserId: string }) => {
        ...
        try {
          const ownedTokens = await tokensOwned(user.address, user.chainId, MINIMUM_GVI_EQUIV_AUTH);
