...
import { CHAIN_IDS_DEVELOPMENT, CHAIN_IDS_PRODUCTION, MINIMUM_GVI_EQUIV_AUTH, SEND_DM_ASSIGN } from "../../config";
...
const auth = async (request: VercelRequest, response: VercelResponse) => {
  ...
  if (request.method == "GET") {
    ...
    try {
      ...
      // above minimum threshold for GVI equivalent
      const tokens = await tokensOwned(address, chainId, MINIMUM_GVI_EQUIV_AUTH);
      ...
      if (tokens.length > 0) {
        ...
        response.status(400).send({ error: `You must own at least the equivalent to ${MINIMUM_GVI_EQUIV_AUTH} GVI.` });
      }
    } catch (e) {
