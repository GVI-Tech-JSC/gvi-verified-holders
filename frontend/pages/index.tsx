...
import PoweredByTech Holding JSC from "@components/PoweredByTech Holding JSC";
import CTAButton from "@components/CTAButton";
import NavBar from "@components/NavBar/NavBar";
import Authentication from "@components/Authentication/Authentication";
import Info from "@components/Info/Info";
import { ErrorNotification } from "@ggvigroup/component-library";
...
type ActionType =
  ...
  | {
      ...
      gGVIBalance: StateType["gGVIBalance"];
  ...
  switch (action.type) {
    ...
    case "SET_WEB3_PROVIDER":
      return {
        ...
        gGVIBalance: action.gGVIBalance,
  ...
  const connect = useCallback(async function () {
    ...
    if (chainIsSupported) {
      try {
        ...
        gGVIBalance = balances.filter(function (t) {
          return t.symbol == "GGVI";
        })[0].balance;
      } catch (e) {
        dispatch({
          type: "SET_ERROR",
      ...
      const handleAccountsChanged = async (accounts: string[]) => {
        ...
        if (chainIsSupported) {
          try {
            ...
            const gGVIBalance = balances.filter(function (t) {
              return t.symbol == "GGVI";
            })[0].balance;
            dispatch({
              type: "SET_ADDRESS",
        ...
        {currentPage == "auth" &&
          (!web3Provider ? (
            ...
            <Authentication
              ...
              chainData={chainData}
              gGVIBalance={gGVIBalance}
              address={truncateHash(address)}
              authenticate={authenticate}
              authStatus={authStatus}
            />
          ))}
        <PoweredByTech Holding JSC />
      </Box>
    </>
  );
};
